...
object Lexer extends RegexParsers {
  ...
  def apply(code: String): Either[String, List[Token]] = {
    parse(tokens, code) match {
      case NoSuccess(msg, next)  => Left(msg + "; before: " + next.source)
...
object LamParser extends PackratParsers {
  ...
  def caseOf: Parser[CaseOf] =
    keyword("case") ~ expr ~ keyword("of") ~ $("{") ~ rep1sep(
      clause,
      $("|")
    ) ~ $("}") ^^ { case _ ~ e ~ _ ~ _ ~ cls ~ _ => CaseOf(e, cls) }
  def let: Parser[Let] =
    keyword("let") ~ identifier ~ $("=") ~ expr ~ keyword(
      "in"
    ) ~ expr ~ keyword("end") ^^ {
  ...
  def pattern: Parser[Pattern] = patUnit | patTup | patCons | patVar
  def patVar: Parser[PatVar] = identifier ^^ PatVar
  def patTup: Parser[PatTuple] =
    $("<") ~ pattern ~ $(",") ~ pattern ~ $(">") ^^ {
      case _ ~ p1 ~ _ ~ p2 ~ _ => PatTuple(p1, p2)
    }
