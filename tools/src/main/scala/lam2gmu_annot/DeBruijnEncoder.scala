...
import lam2gmu_annot.ASTs.{ConcreteSyntax => CS, DeBruijnSyntax => DS}
...
object DeBruijnEncoder {
  ...
  def encSub(
    ...
    expresssion match {
      ...
      case CS.VTuple(fst, fstTyp, snd, sndTyp) =>
        DS.VTuple(
          encSub(fst, exprSubst, typSubst),
          encSubT(fstTyp, typSubst),
          encSub(snd, exprSubst, typSubst),
          encSubT(sndTyp, typSubst),
        )
      case CS.Fst(e) =>
      ...
      case CS.CaseOf(e, name, clauses, retT) =>
        DS.CaseOf(
          encSub(e, exprSubst, typSubst),
          name,
          clauses.map {
  ...
  def encPat(
    ...
    pattern match {
      case CS.PatConstructor(name, args, bodyName) =>
        ...
        val (bodyPat, eS, tS) = (DS.Binder, exprSubst.enterBinder(bodyName), tP)
