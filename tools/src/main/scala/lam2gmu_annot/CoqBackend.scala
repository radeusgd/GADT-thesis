...
case class CoqBackend(sigma: Sigma) {
  ...
  def renderExpr(expression: Expression): String =
    expression match {
      ...
      case VUnit => "trm_unit"
      case VTuple(fst, fstTyp, snd, sndTyp) =>
        s"trm_tuple (${renderTyp(fstTyp)}) (${renderExpr(fst)}) (${renderTyp(sndTyp)}) (${renderExpr(snd)})"
      case Fst(e) =>
      ...
      case CaseOf(e, _, clauses, retType) =>
        val (gadtName, cls) = renderClauses(clauses)
        val clsR = cls.reverse.mkString("[", "; ", "]*")
        s"trm_matchgadt (${renderExpr(e)}) $gadtName $clsR (${renderTyp(retType)})"
  ...
  def renderClauses(clauses: Seq[Clause]): (String, Seq[String]) = {
    ...
    def unwrapPat(clause: Clause): PatData =
      clause.pattern match {
        case PatConstructor(name, args, Binder) =>
